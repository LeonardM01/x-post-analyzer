...
export function generateReport(analysis) {
  ...
  lines.push('');
  lines.push(`> Based on the [open-source X algorithm](https://github.com/xai-org/x-algorithm)`);
  ...
  if (slop.is_likely_ai) {
    ...
    lines.push('> Repeated AI-pattern content can suppress your account-level reach, not just this tweet.');
    lines.push('');
  ...
  lines.push('');
  lines.push('Slop suppresses banger likelihood and triggers negative-feedback heads:');
  lines.push('');
  lines.push('```');
  lines.push('not_interested  → suppresses tweet in the user\'s feed');
  lines.push('block_author    → tweet hidden + author downweighted for that user');
  lines.push('mute_author     → tweet hidden for that user');
  lines.push('report          → strong negative signal across the platform');
  lines.push('```');
  lines.push('');
  lines.push('A high banger score (>= 0.4) is upstream\'s positive-quality counterweight.');
  lines.push('Reducing slop-like patterns improves both axes simultaneously.');
  lines.push('');
  ...
  for (const item of ep.breakdown) {
    ...
    let weightDisplay;
    if (item.weight_source === 'legacy_2023') {
      weightDisplay = `${item.effective_weight}*`;
    } else if (item.weight_source === 'current_2026') {
      weightDisplay = item.effective_weight;
    } else {
      weightDisplay = 'unpublished';
    }
    lines.push(`| ${item.description} | ${item.probability} | ${weightDisplay} | ${sign}${item.contribution} |`);
  }
  lines.push('');
  ...
  lines.push(checkItem(!analysis.issues.some((i) => i.severity === 'high' || i.severity === 'critical'), 'No critical issues'));
  ...
  lines.push('');
  lines.push('---');
  lines.push('*Generated by x-post-analyzer — Open-source X algorithm analysis tool. Based on [xai-org/x-algorithm](https://github.com/xai-org/x-algorithm).*');
  lines.push('');
